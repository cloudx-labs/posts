...
The catch: a single byte change breaks it. The cache only kicks in when the prefix is exactly identical to a previous request.

A classic example is injecting the current timestamp into your system prompt. Seems harmless, but now every call is unique, the cache never hits, and you're paying full price every time.
