...
This series assumes you have a basic understanding of the smart contracts development and the Ethereum or EVM compatible blockchains, also have a local development setup already working. If you are not familiar with these topics, please read the [Introduction to smart contracts](https://ethereum.org/en/smart-contracts/) to start.
...
We are going to be focussing in creating a [payments gateway](https://en.wikipedia.org/wiki/Payment_gateway), by using the ERC20 standard as the basis for our tokens. This will allow us to share the implementation in other applications that support the standard interface as the popular stable coins: [USDC](https://www.circle.com/en/usdc), [USDT](https://tether.to/en/), [DAI](https://makerdao.com/en/).

### _"The immutable problem"_

The pure concept of smart contracts is that they are immutable. Once a contract is deployed, it cannot be changed. This is a great feature because it allows us to trust the code that is running in the blockchain. We can be sure that the code will not change and that it will behave as expected. This means that if we want to add new features or fix something broken in the contracts, we need to deploy a new contract and migrate the state to the new one, this is not a problem for a new contract, but it is a big problem for an existing with a lot of users interacting with it.

That's why the concept of "upgradeable contracts" was created. The idea is to have a contract that can be upgraded without losing the state. This is a very complex topic and there are many different approaches to it. We are going to be using the [OpenZeppelin Upgrades](https://docs.openzeppelin.com/upgrades-plugins/1.x/) library to create our upgradeable contracts.

### **How the proxy pattern works?**

The secret behinds a proxy consists in a contract that provides a fallback function that delegates all calls to another contract using the EVM instruction `delegatecall`, this is a low-level function that allows calling another contract using the current contract **storage** and **context**. This means that the contract that is being called can access the storage of the contract that is calling it. This is the key to the proxy pattern.
...
1. Initializers: Contructors are not allowed in upgradeable contracts, instead we need to use the `initialize` function from the `Initializable` contract. This function is called only once when the contract is deployed for the first time. This means that we need to use the `initializer` modifier in the `initialize` function.
...
To give the ability of upgradeability to our "Gateway" (in case something goes wrong or need to change), we need to use the proxy pattern. So let's start by creating and deploying a `Gateway.sol` contract:
...
To deploy the contract we need to use the `deployProxy` function from the `@openzeppelin/hardhat-upgrades` plugin. This function will deploy the proxy contract and initialize it with the `initialize` function from the `Gateway` contract.
...
Why specifying the network? Because we need to tell hardhat which network to use to interact with the blockchain, in this case we are using the `sepolia` network, also we will use `ethersjs` to interact with the deployed contract.
...
As you can see, we are able to interact with the `Gateway` contract via the `TransparentUpgradeableProxy` contract, but what if we want to upgrade the `Gateway` contract? Let's add support to allows the owner to change the fee, so let's create `GatewayV2.sol`:
